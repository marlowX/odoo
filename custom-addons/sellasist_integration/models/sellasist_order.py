...
class SellasistOrder(models.Model):
    ...
    def sync_orders_from_sellasist(self, hours_back=24):
        """
        POPRAWIONA METODA - rozdziela aktualizacje od nowych zamówień
        """
        ...
        try:
            date_to = self.get_localized_datetime()
            date_from = date_to - timedelta(hours=hours_back)
            
            _logger.info(f"🚀 Synchronizacja {date_from} → {date_to}")
            
            # 1. AKTUALIZACJE z orders_logs
            updated_count = self._sync_updates_only(config, date_from, date_to)
            
            # 2. NOWE ZAMÓWIENIA z orders
            new_count = self._sync_new_only(config, date_from, date_to)
            
            _logger.info(f"✅ Gotowe: {updated_count} aktualizacji, {new_count} nowych")
            return True
            
        except Exception as e:
            _logger.error(f"❌ Błąd synchronizacji: {str(e)}")
            return False
    ...
    def _prepare_order_vals(self, data):
        ...
        def safe_datetime(date_str):
            if date_str:
                try:
                    import pytz
                    # Parsuj datę jako UTC (jak wysyła Sellasist)
                    ...
                    local_dt = utc_dt.astimezone(poland_tz)
                    # Zwróć bez strefy czasowej
    ...
    def _create_invoice(self, sale_order):
        ...
        try:
            ...
            # NAJPIERW dodaj linie do zamówienia jeśli ich brak
            if not sale_order.order_line:
                _logger.warning(f"Brak linii w zamówieniu {sale_order.name} - pomijam fakturę")
                return
            ...
            # DODAJ linie faktury z zamówienia
            invoice_lines = []
            ...
            # Potwierdź fakturę jeśli opłacona
            if self.payment_status == 'paid':
                invoice.action_post()
    ...
    def _sync_updates_only(self, config, date_from, date_to):
        """TYLKO aktualizacje istniejących zamówień"""
        try:
            ...
            _logger.info("📝 Pobieram logi zmian (orders_logs)...")
            response = requests.get('https://alpma.sellasist.pl/api/v1/orders_logs', headers=headers, params=params, timeout=30)
            
            if response.status_code != 200:
                _logger.error(f"❌ Błąd orders_logs: {response.status_code}")
                return 0
                
            logs = response.json()
            if not logs:
                _logger.info("✅ Brak zmian")
                return 0
                
            # Tylko zamówienia które JUŻ MAMY
            changed_ids = list(set([log.get('order_id') for log in logs if log.get('order_id')]))
            existing_orders = self.search([('sellasist_id', 'in', [str(oid) for oid in changed_ids])])
            existing_ids = [int(order.sellasist_id) for order in existing_orders]
            
            _logger.info(f"🔄 Aktualizuję {len(existing_ids)} zamówień")
            ...
            _logger.error(f"❌ Błąd aktualizacji: {str(e)}")
            return 0

    def _sync_new_only(self, config, date_from, date_to):
        """TYLKO nowe zamówienia z paginacją"""
        try:
            headers = {'accept': 'application/json', 'apiKey': config.api_key}
            new_count = 0
            page = 1
            ...
            while True:
                ...
                _logger.info(f"📄 Strona {page} nowych zamówień...")
                response = requests.get('https://alpma.sellasist.pl/api/v1/orders', headers=headers, params=params, timeout=30)
                
                if response.status_code != 200:
                    _logger.error(f"❌ Błąd orders strona {page}: {response.status_code}")
                    break
                    
                orders = response.json()
                if not orders:
                    _logger.info(f"🏁 Koniec na stronie {page}")
                    break
                
                # Tylko NOWE zamówienia
                order_ids = [o.get('id') for o in orders if o.get('id')]
                existing = self.search([('sellasist_id', 'in', [str(oid) for oid in order_ids])])
                existing_ids = set([int(o.sellasist_id) for o in existing])
                new_ids = [oid for oid in order_ids if oid not in existing_ids]
                
                _logger.info(f"🆕 Strona {page}: {len(new_ids)} nowych z {len(orders)} łącznie")
            ...
            _logger.error(f"❌ Błąd nowych zamówień: {str(e)}")
            return 0

    @api.model
    def sync_orders_by_date_range(self, date_from, date_to):
        """
        Synchronizacja dla konkretnego zakresu dat (z wizarda)
        """
        _logger.info("=" * 60)
        _logger.info(f"🗓️ SYNCHRONIZACJA NIESTANDARDOWEGO ZAKRESU")
        _logger.info(f"📅 Od: {date_from}")
        _logger.info(f"📅 Do: {date_to}")
        _logger.info("=" * 60)
        ...
        try:
            # 1. AKTUALIZACJE - orders_logs 
            _logger.info("📝 ETAP 1: AKTUALIZACJE (orders_logs)")
            updated_count = self._sync_order_updates(config, date_from, date_to)
            
            # 2. NOWE ZAMÓWIENIA - orders
            _logger.info("🆕 ETAP 2: NOWE ZAMÓWIENIA (orders)")
            new_count = self._sync_new_orders_by_date_range(config, date_from, date_to)
            
            _logger.info("=" * 60)
            _logger.info(f"✅ NIESTANDARDOWY ZAKRES ZAKOŃCZONY:")
            _logger.info(f"   🔄 Aktualizacji: {updated_count}")
            _logger.info(f"   ✨ Nowych: {new_count}")
            _logger.info("=" * 60)
            return True
            
        except Exception as e:
            _logger.error(f"❌ BŁĄD SYNCHRONIZACJI ZAKRESU: {str(e)}")
            return False

    @api.model
    def sync_orders_by_date_range(self, date_from, date_to):
        """
        Synchronizacja dla KONKRETNEGO zakresu dat (nie wstecz od teraz)
        """
        _logger.info("=" * 60)
        _logger.info(f"🗓️ SYNCHRONIZACJA KONKRETNEGO ZAKRESU")
        _logger.info(f"📅 Od: {date_from}")
        _logger.info(f"📅 Do: {date_to}")
        _logger.info("=" * 60)
        ...
        try:
            # 1. AKTUALIZACJE - orders_logs dla tego zakresu
            updated_count = self._sync_order_updates(config, date_from, date_to)
            
            # 2. NOWE ZAMÓWIENIA - orders dla tego zakresu
            new_count = self._sync_new_orders_by_date_range(config, date_from, date_to)
            
            _logger.info("=" * 60)
            _logger.info(f"✅ ZAKRES {date_from} → {date_to} ZAKOŃCZONY:")
            _logger.info(f"   🔄 Aktualizacji: {updated_count}")
            _logger.info(f"   ✨ Nowych: {new_count}")
            _logger.info("=" * 60)
            return True
    ...
    @api.model
    @api.model
    def get_localized_datetime(self, dt=None):
        """Pobierz czas z uwzględnieniem strefy czasowej Polski"""
        import pytz
        
        if dt is None:
            # Użyj UTC i konwertuj na czas polski
            ...
            # Konwertuj na czas polski
            dt = utc_now.astimezone(poland_tz)
        
        # Zwróć bez informacji o strefie czasowej
        return dt.replace(tzinfo=None)

    def _get_local_time(self, utc_time=None):
        ...
        # Jeśli czas nie ma strefy, dodaj UTC
        if utc_time.tzinfo is None:
            utc_time = utc.localize(utc_time)
        
        # Konwertuj na czas polski
        local_time = utc_time.astimezone(poland_tz)
        
        # Zwróć bez informacji o strefie (dla API)
        return local_time.replace(tzinfo=None)

    @api.depends('date')
    def _compute_display_date(self):
        """Oblicz datę do wyświetlenia w czasie polskim"""
        import pytz
        for record in self:
            if record.date:
                # Jeśli data nie ma strefy czasowej, traktuj jako UTC
                ...
                # Konwertuj na czas polski
                poland_tz = pytz.timezone('Europe/Warsaw')
                local_date = utc_date.astimezone(poland_tz)
                record.display_date = local_date.replace(tzinfo=None)
            else:
                record.display_date = False

    display_date = fields.Datetime('Data zamówienia (lokalny czas)', compute='_compute_display_date', store=False)

    def _convert_sellasist_date_to_local(self, date_str):
        """Konwertuj datę z Sellasist (UTC) na lokalny czas polski"""
        if not date_str:
            return None
        try:
            import pytz
            # Parsuj datę UTC z Sellasist
